...
TOOLS=[dict(type='function',name=name,description=description,strict=True,parameters=schema) for name,description,schema in [
    ('get_plan_summary','Показатели, ограничения и основания выбранной версии плана.',object_schema({'plan_id':STRING})),
    ('get_campaign_evidence','Паспорт одной кампании и расчёт замены канала в целом плане. Индекс начинается с нуля.',object_schema({'plan_id':STRING,'campaign_index':{'type':'integer','minimum':0,'maximum':9}})),
