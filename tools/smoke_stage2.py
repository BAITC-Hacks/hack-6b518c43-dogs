...
def main():
    ...
    report=dict(source='Реальная локальная HTTP-проверка',external_ai_tested=False,run_id=rid,
        ...
        checks=['Отсутствующий ключ','Ручной пересчёт','Отдельное предложение','База сравнения','Явное применение',
    ...
    (ROOT/'reports/stage2/after/api_stage2.json').write_text(json.dumps(report,ensure_ascii=False,indent=2));print(json.dumps(report,ensure_ascii=False))
